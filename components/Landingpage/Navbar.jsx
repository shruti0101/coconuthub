...
const textColor = scroll ? "text-white" : isProductPage ? "text-black" : "text-white";
  ...
  return (
    <header
      className={`fixed top-0 left-0 z-50 w-full transition-all duration-300 ${
        scroll ? "bg-black/60 backdrop-blur-md" : "bg-black/30"
      }`}
    >
            ...
            if (item.name === "PRODUCTS") {
              return (
                <div key={item.name} className="relative" ref={dropdownRef}>
                  ...
                  {productsOpen && (
                    <div className="absolute top-10 left-0 bg-white text-black shadow-xl rounded-lg w-56 p-4 space-y-2 z-50">
                      {categories.map((cat) => (
                        <Link
                          key={cat.id}
  ...
  <Search
    ...
    className={`absolute left-3 ${
      scroll ? "text-white" : isProductPage ? "text-black" : "text-white"
    }`}
  />
